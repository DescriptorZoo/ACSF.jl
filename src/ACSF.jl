...
function G2(dj, Rc, eta; Rs=0.0)
    ...
    exp_arg = (dj .- Rs).^2
    return [sum(exp.(-eta[p] .* exp_arg) .* fc_ij) for p=1:length(eta)]
end
...
function G4(Rj, dj, Rc, eta, lambd, zeta)
    ...
    return [2.0.^(1.0 - zeta[p]) .* 
            sum( 
                (1.0 .+ lambd[p] .* cos_theta_ijk[jkmask]).^zeta[p] .*
...
function acsf_desc(Rs; Gparams=nothing)
    ...
    if "G2" in keys(Gparams)
        ...
        descriptor = vcat(descriptor,G2(ds, Gparams["G2"]))
    end
    if "G4" in keys(Gparams)
        # Concatenate angular basis functions
        descriptor = vcat(descriptor,G4(Rs, ds, Gparams["G4"])) 
    end
    return descriptor
end
